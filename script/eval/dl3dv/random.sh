...
# DL3DV "Ours w/o Condenser and K-means" baseline: the first-stage
# encoder-decoder with random token selection at each storage ratio.
#
# Usage: bash script/eval/dl3dv/random.sh [num_context_views]
...
do
    echo "Running DL3DV random-selection evaluation for ${num_context_views} context views, token ratio: $ratio"
    python eval.py \
        ...
        ckpt_path=$ckpt_path \
        inference_dir=dl3dv_random_${num_context_views}v_ratio_${ratio} \
        save_images=false \
        save_videos=false
done
